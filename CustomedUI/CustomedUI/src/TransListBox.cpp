...
:  m_ItemHeight(20),
   ...
   m_Shadow(FALSE),
   m_ShadowOffset(3), m_bReadOnly(FALSE)
{
...
void CTransListBox::SetFont(int nPointSize, CString FaceName,COLORREF Color,COLORREF SelColor,BOOL Shadow,int SOffset,COLORREF ShadowColor)
{
	...
	CDC *DC = GetDC();
	m_Font.DeleteObject();
//	m_Font.CreateFont(nPointSize,0,0,0,NULL,FALSE,FALSE,FALSE,ANSI_CHARSET,OUT_CHARACTER_PRECIS,CLIP_CHARACTER_PRECIS,PROOF_QUALITY,DEFAULT_PITCH,FaceName);
	
	m_Font.CreateFont(nPointSize, 0, 0, 0, NULL, FALSE, FALSE, FALSE,DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, ANTIALIASED_QUALITY, DEFAULT_PITCH, FaceName);
...
void CTransListBox::SetColor(COLORREF Color,COLORREF SelColor, BOOL Shadow, int SOffset, COLORREF ShadowColor)
{
	m_Shadow = Shadow;
	m_ShadowColor = ShadowColor;
	m_ShadowOffset = SOffset;
...
void CTransListBox::SetReadOnly(BOOL bTrue /*= TRUE*/)
{
	m_bReadOnly = bTrue;
}
...
void CTransListBox::OnMButtonDown(UINT nFlags, CPoint point)
{
	if(m_bReadOnly){
		return;
	}
...
void CTransListBox::OnLButtonDown(UINT nFlags, CPoint point)
{
	if(m_bReadOnly){
		return;
	}
